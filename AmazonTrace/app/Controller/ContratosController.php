...
class ContratosController extends AppController {
    ...
    public function index($filtro = NULL, $pesquisa = NULL) {
        ...
        if ($filtro && $pesquisa) {
            ...
            if ($filtro == 'nome') {
                ...
                $this->paginate = array('fields' => $fields, 'limit' => 50, 'conditions' => array('Cliente.' . $filtro . ' LIKE' => '%' . $pesquisa . '%'));

                $contratos = $this->Contrato->find('all', array('fields' => $fields, 'conditions' => array('Cliente.' . $filtro . ' LIKE' => '%' . $pesquisa . '%')));
            } else {

                $this->paginate = array('fields' => $fields, 'limit' => 50, 'conditions' => array('Contrato.' . $filtro . ' LIKE' => '%' . $pesquisa . '%'));

                $contratos = $this->Contrato->find('all', array('fields' => $fields, 'conditions' => array('Contrato.' . $filtro . ' LIKE' => '%' . $pesquisa . '%')));
            }
        }
    ...
    public function delete($id = null) {
        ...
        $arrayMensalidades = $this->Contrato->Mensalidade->find('list', array('conditions' => array('Mensalidade.contrato_id' => $id)));

        foreach ($arrayMensalidades as $value) {
            $this->requestAction(array("controller" => "Mensalidades", "action" => "delete", $value));
        }
